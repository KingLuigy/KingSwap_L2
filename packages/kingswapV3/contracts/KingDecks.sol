...
contract KingDecks is Ownable, ReentrancyGuard, TokenList {
    ...
    function computeEarlyWithdrawal(
        ...
        (amountToUser, fees, ) = _computeEarlyWithdrawal(_deposit, tS, block.timestamp);
    }
    ...
    function _withdraw(uint256 depositId, bool isInterim) internal {
        ...
        if (isInterim) {
            TermSheet memory tS = _termSheets[termsId - 1];
            require(
                block.timestamp >= uint256(_deposit.lastWithdrawTime) + tS.minInterimHours * 3600,
            ...
            require(block.timestamp >= _deposit.maturityTime, "KDecks:deposit is locked");
