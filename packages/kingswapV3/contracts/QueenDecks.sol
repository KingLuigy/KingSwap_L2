...
contract QueenDecks is Ownable, ReentrancyGuard {
        ...
        returns (uint256 totalDue, uint256 rewardDue)
    {
        ...
        (totalDue, rewardDue) = _amountDueOn(stake, block.timestamp);
    }
    ...
    function deposit(uint256 termsId, uint256 amount) public nonReentrant {
        ...
        uint32 unlockTime = safe32(block.timestamp.add(uint256(tS.lockHours) * 3600));

        uint256 stakeId = _encodeStakeId(tS.token, stakeNum, block.timestamp, tS.lockHours);
    ...
    function _withdraw(uint256 stakeId, bool isEmergency) internal {
        ...
        (uint256 amountDue, uint256 reward) = _amountDueOn(stake, block.timestamp);
        ...
        if (isEmergency) {
            ...
            require(block.timestamp >= stake.unlockTime, "withdraw: stake is locked");
            amountToUser = amountDue;
            emit Withdraw(msg.sender, stakeId, amountToUser);
        }

        if (block.timestamp > stake.lastRewardTime) {
            userStakes.data[stakeId].lastRewardTime = safe32(block.timestamp);
        }
    ...
    function _withdrawReward(uint256 stakeId) internal {
        ...
        uint256 allowedTime = stake.lastRewardTime +
            ...
            3600;
        require(block.timestamp >= allowedTime, "QDeck:reward withdrawal not yet allowed");

        (, uint256 reward) = _amountDueOn(stake, block.timestamp);
        if (reward == 0) return;

        stakes[msg.sender].data[stakeId].lastRewardTime = safe32(block.timestamp);
