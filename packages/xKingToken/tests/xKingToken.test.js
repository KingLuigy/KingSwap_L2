...
// Source (adopted): openzeppelin-contracts/test/token/ERC20
...
contract('ERC20', function (accounts) {
  ...
  const name = 'x$KING Token';
  const symbol = 'x$KING';

  const initialSupply = new BN(100);

  beforeEach(async function () {
    this.token = await XKingToken.new();
    await this.token._mintMock(initialHolder, initialSupply);
  });
        ...
        describe('when there was no approved amount before', function () {
          it('reverts', async function () {
            await expectRevert(this.token.decreaseAllowance(
                spender, amount, { from: initialHolder }), 'ERC20: allowance exceeded',
            );
          });
        });
  ...
  describe('increase allowance', function () {
    ...
    describe('when the spender is the zero address', function () {
      ...
      it('reverts', async function () {
        await expectRevert(
            this.token.increaseAllowance(spender, amount, { from: initialHolder }), 'ERC20: approve to zero address',
        );
      });
    });
  });
  ...
  describe('_mint', function () {
    ...
    it('rejects a null account', async function () {
      await expectRevert(
          this.token._mintMock(ZERO_ADDRESS, amount), 'ERC20: mint to zero address',
      );
    });

    describe('for a non zero account', function () {
      beforeEach('minting', async function () {
        const { logs } = await this.token._mintMock(recipient, amount);
        this.logs = logs;
      });
  ...
  describe('_burn', function () {
    it('rejects a null account', async function () {
      await expectRevert.unspecified(this.token._burnMock(ZERO_ADDRESS, new BN(1)));
    });

    describe('for a non zero account', function () {
      it('rejects burning more than balance', async function () {
        await expectRevert.unspecified(this.token._burnMock(initialHolder, initialSupply.addn(1)));
      });

      const describeBurn = function (description, amount) {
        describe(description, function () {
          beforeEach('burning', async function () {
            const { logs } = await this.token._burnMock(initialHolder, amount);
            this.logs = logs;
          });
